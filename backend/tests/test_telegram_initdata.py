...
def build_login_widget_style_init_data(payload: dict[str, str], bot_token: str = BOT_TOKEN) -> str:
    data = {key: str(value) for key, value in payload.items()}
    data_check_string = "\n".join(f"{key}={data[key]}" for key in sorted(data))
...
def test_verify_init_data_rejects_login_widget_signature_model() -> None:
    auth_date = str(int(time.time()))
    user_json = json.dumps({"id": 123, "first_name": "Ada"})
    init_data = build_login_widget_style_init_data({"auth_date": auth_date, "user": user_json})
