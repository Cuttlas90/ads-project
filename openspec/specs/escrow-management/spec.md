...
The system SHALL persist escrows in a `deal_escrows` table with fields `id`, `deal_id` (FK to `deals.id`, unique), `state` (required, indexed), `deposit_address` (nullable, unique), `expected_amount_ton` (nullable decimal), `received_amount_ton` (nullable decimal), `deposit_tx_hash` (nullable string, indexed), `deposit_confirmations` (int default 0), `fee_percent` (required decimal), `created_at`, and `updated_at`. It SHALL set `expected_amount_ton = deal.price_ton` at escrow creation, set `fee_percent` as a snapshot from settings, and enforce one escrow per deal.
